...
func (ut *unaryThreshold) Extend(choice int) binaryThreshold {
	...
	bt.choiceConfidence[0] = make([]int, len(ut.terminationConditions))
	bt.choiceConfidence[1] = make([]int, len(ut.terminationConditions))
	// Copy unary confidence to choice 1 (the only choice in unary)
	copy(bt.choiceConfidence[1], ut.confidence)
	return bt
}
