...
// finality with both BLS (classical) and Corona (quantum) security.
...
func main() {
	...
	rtKeyPair, err := corona.GetOrCreateKeyPair("~/.lux")
	if err != nil {
		log.Fatal(err)
	}
	...
	validators := &ExampleValidatorSet{
		validators: map[ids.NodeID]*corona.Validator{
			nodeID: {
				NodeID:    nodeID,
				BLSPubKey: blsKey,
	...
	network := &MockNetwork{
		shares: make(chan *corona.Share, 100),
		certs:  make(chan *corona.Certificate, 10),
	}
	
	service := corona.NewQuasarService(nodeID, rtKeyPair, validators, network)
	...
	fmt.Println("Both classical (BLS) and quantum (Corona) security active.")
}
...
func (vs *ExampleValidatorSet) GetValidator(id ids.NodeID) (*corona.Validator, error) {
...
type MockNetwork struct {
	shares chan *corona.Share
	certs  chan *corona.Certificate
}

func (n *MockNetwork) BroadcastShare(share *corona.Share) error {
	select {
	case n.shares <- share:
		return nil
	default:
...
func (n *MockNetwork) SendCertificate(cert *corona.Certificate) error {
	select {
	case n.certs <- cert:
		return nil
	default:
...
func (n *MockNetwork) SubscribeShares() <-chan *corona.Share {
	return n.shares
}

func (n *MockNetwork) SubscribeCertificates() <-chan *corona.Certificate {
	return n.certs
}