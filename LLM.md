...
| `quasar` | BLS + Corona + ML-DSA threshold signing | `signer`, `BLS`, `EpochManager`, `BundleSigner` |
...
type QuasarCert struct {
    ...
    Corona   []byte  // Ring-LWE threshold (PQ), O(1) after DKG
...
| Corona | Ring-LWE threshold | Module-LWE | O(1) after DKG | variable |

Modes (each layer independently toggleable):
- BLS-only: classical fast path
- BLS + Corona: dual PQ
- BLS + Corona + ML-DSA: full Quasar (`TripleSignRound1`)
...
| `BLSPlusCorona` | bls-rt | BLS + Corona 2-round threshold |
...
with real signing (real BLS aggregate, real ML-DSA-65, real Corona
2-round threshold).
...
| **Q-Chain** | Runs the Corona 2-round threshold for *consensus only* (this repo's `protocol/quasar/` emits those rounds). Not a general MPC host. |
| **T-Chain** | Runs *all* MPC ceremonies: CGGMP21, FROST, Corona (general), TFHE. The signing partner for cross-chain custody. |
...
- `~/work/lux/proofs/quasar-cert-soundness.tex`:
  ...
  - App E — Corona parameter tightness: classical 2^142, quantum 2^130 via
...
| Quasar full block (BLS+ML-DSA+Corona) | 1.85 ms | `protocol/quasar BenchmarkQuasarBlockProcessing` |
...
- Corona threshold verify: variable, amortized O(1) after DKG
...
- All tests pass except `TestQuantumBundle_ChainIntegrity` which is flaky
  (Corona threshold signing nondeterminism -- passes on retry)
