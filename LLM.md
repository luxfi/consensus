...
- `config/pq_mode.go` — `PQMode` enum (`bls`, `ringtail`, `pulsar`,
...
type QuasarCert struct {
    ...
    Ringtail   []byte    // Lattice threshold slot — under strict-PQ this is Pulsar / Pulsar-M
...
| `Ringtail` | Pulsar (R-LWE threshold) for Q-Chain consensus OR Pulsar-M (M-LWE threshold, output verifies under unmodified FIPS 204 ML-DSA.Verify) for HIP-0079 Q-blocks | same as strict-PQ; threshold-lattice is the same layer either way |
...
| `PQModeRingtail` | `ringtail` | BLS + Ringtail academic (BLAKE3); trusted-dealer DKG, federation-only |
