...
func (ts *Topological) Initialize(
	...
	// params validation removed - config.Parameters doesn't have Verify method
...
func (ts *Topological) Health(ctx context.Context) (health.Health, error) {
	details, err := ts.HealthCheck(ctx)
	if err != nil {
		return health.Health{Healthy: false, Details: err}, err
	}
...
func (ts *Topological) acceptPreferredChild(ctx context.Context, n *chainBlock) error {
	...
	for childID, child := range n.children {
		...
		ts.ctx.Log.Trace("rejecting block",
			"reason", "conflict with accepted block",
			"blkID", childID,
			"height", child.Height(),
			"conflictID", pref,
		)
