...
func (c *ChainConsensus) AddBlock(ctx context.Context, block *Block) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	fmt.Fprintf(os.Stderr, "[ADD-DEBUG] AddBlock id=%s existing=%v self=%p\n", block.id, c.blocks[block.id] != nil, c)
...
func (c *ChainConsensus) Poll(ctx context.Context, responses map[ids.ID]int) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	fmt.Fprintf(os.Stderr, "[POLL-DEBUG] Poll called: responses=%v alpha=%d self=%p blocksLen=%d\n", responses, c.alpha, c, len(c.blocks))

	// Poll each block's Lux consensus instance using Wave → Focus protocols
	for blockID, votes := range responses {
		block, exists := c.blocks[blockID]
		fmt.Fprintf(os.Stderr, "[POLL-DEBUG] checking blockID=%s votes=%d exists=%v\n", blockID, votes, exists)
		if !exists {
			for k := range c.blocks {
				fmt.Fprintf(os.Stderr, "[POLL-DEBUG]   have block: %s (match=%v)\n", k, k == blockID)
			}
			continue
		}
