...
func (t *Transitive) buildSingleValidatorCertLocked(pending *PendingBlock, blockID ids.ID) VerifiedQuorumCert {
	...
	// No vote crypto wired (dev single-node): synthesize the 1-of-1 finality
	// witness from the position. ForceAccept (guarded) already decided it.
	pos := t.blockPositionLocked(pending, blockID)
...
func (t *Transitive) buildBlocksLocked(ctx context.Context) error {
	...
	for t.pendingBuildBlocks > 0 {
		...
		// single-node case where consensus.IsAccepted already flipped true on the
		// self-vote. We no longer VM.Accept inline here: finalization for every
		// block (K=1 or K>1) goes through the SOLE cert-gated finalizer via
		// TryAccept below, so there is exactly one acceptance road. In K=1
		// TryAccept commits the 1-of-1 quorum (ForceAccept is idempotent if the
		// self-vote already accepted) and finalizes through AcceptWithCert.
		...
		// Proposer-self-finalize trigger: attempt finalization of the just-built
		// own block through the SOLE cert-gated path (TryAccept via
		// finalizeOwnProposal). The block was verified locally above, so the
		// proposer has committed to its correctness; waiting only on peer Chits to
		// drive the local alpha-of-K threshold causes the lux-devnet stall when
		// Chits arrive late or are dropped. See finalizeOwnProposal / TryAccept for
		// the safety argument — K>1 finalizes ONLY with a verified α-of-K cert
		// (never on the lone self-vote), K=1 finalizes the 1-of-1 quorum.
		//
		// We attempt this whenever the block is finalizable now: K=1 (the self-vote
		// IS the quorum — alreadyAccepted) OR a network proposer has broadcast the
		// proposal (K>1 — the cert may already be assemblable, else TryAccept
		// no-ops and the poll loop retries). TryAccept is idempotent and takes its
		// own lock.
