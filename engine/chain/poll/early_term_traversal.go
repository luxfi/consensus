...
func (p *earlyTermPoll) Finished() bool {
	...
	// linear would instantiate a unary threshold instance on the prefix of 'u' and 'w'.
	...
	// We wish to compute the votes for threshold instances, no matter if they correspond to an actual block ID,
	// or a unary threshold instance for a shared prefix between a bifurcation of two competing blocks.
	// For that, only the number of votes and existence of such threshold instances matters.
	voteCountsForIDsOrPrefixes := aggregateVotesFromPrefixesAndIDs(transitiveVotesForPrefixes, transitiveVotes)

	// Given the aforementioned votes, we wish to see whether there exists a threshold instance
	// that can benefit from waiting for more invocations of Vote().
	// We therefore check each amount of votes separately and see if voting for that threshold instance
...
func (p *earlyTermPoll) Result() (ids.ID, bool) {
	if !p.finished {
	...
	return bestID, maxVotes >= p.alphaConfidence
}
