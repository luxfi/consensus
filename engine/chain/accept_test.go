...
// within d. It is the LIVENESS assertion: a ⅔ supermajority — more than two thirds of the
// signing stake, from at least TwoThirdsCount(n) signers — DOES drive VM.Accept. That is
// the one certificate a block is accepted on, and the same one it is exported on.
func mustFinalize(t *testing.T, e *Transitive, blk *verifyOnceBlock, d time.Duration, why string) {
	...
	if !waitFor(d, func() bool { return e.IsAccepted(blk.id) }) {
		t.Fatalf("%s: LIVENESS FAILURE — a ⅔ quorum did not accept (AcceptCalled=%d)", why, blk.AcceptCalled())
	}
...
// export LIVENESS assertion: a real >⅔-stake supermajority DOES certify for export.
...
// export-SAFETY assertion: a coalition below the ⅔ supermajority reaches neither
// acceptance nor export finality (bridges / DEX settlement / cross-chain), and the Quasar
// frontier is NOT advanced to its block.
...
// validators sign, holding 4 of 100 stake. A block is accepted only on more than two thirds
// of the signing stake, so this coalition gets neither acceptance nor export.
func TestHeadcountMajorityWithoutStakeAcceptsNothing(t *testing.T) {
	...
	mustNotFinalize(t, e, blk, 2*time.Second, "4/100-stake coalition (accept)")
...
func TestSkewedStakeHeadcountMajorityRejected(t *testing.T) {
	...
	mustNotFinalize(t, e, blk, 2*time.Second, "40%-stake coalition (accept)")
...
// TestThreeOfFiveAcceptsNothingTheFourthAcceptsAndExports: three of five sign — a bare
// majority, 60% of equal stake, below ⅔ — and the block is neither accepted nor exported:
// one equivocator could put a majority on each of two blocks at one height. The fourth
// signature carries it past ⅔, and the block is accepted and exported in the same step.
func TestThreeOfFiveAcceptsNothingTheFourthAcceptsAndExports(t *testing.T) {
	vs := newTestValidatorSet(5)
	equal := newStakeMap(vs, 20, 20, 20, 20, 20) // three → 60% < ⅔
	rec := &recordingGossiper{}

	blk := newTestBlock(1, ids.Empty, "three-of-five")
	e, chainID := driveSignedAccepts(t, vs, equal, rec, blk, []int{0, 1, 2})

	mustNotFinalize(t, e, blk, 2*time.Second, "3/5 bare majority (accept)")
	mustNotQuasar(t, e, blk, 500*time.Millisecond, "3/5 = 60% stake (export gate)")

	pos := VotePosition{ChainID: chainID, Height: blk.height, Round: 0, BlockID: blk.id, ParentID: blk.parentID}
	e.ReceiveVote(vs.signedVote(3, pos))
	mustFinalize(t, e, blk, 2*time.Second, "4/5 = 80% stake (accept)")
	mustQuasar(t, e, blk, 2*time.Second, "4/5 = 80% stake (export)")
}

// TestVoteArrivalWithoutStakeAcceptsNothingUntilTheHeavyVote: a vote arriving is a LIVENESS
// trigger into TryAccept, never an authority. B,C,D,E (4/5 count, 40% stake) sign and nothing
// is accepted: acceptance needs more than two thirds of the signing stake. When the heavy
// holder A (60%) signs, the block is accepted and exported in the same step.
func TestVoteArrivalWithoutStakeAcceptsNothingUntilTheHeavyVote(t *testing.T) {
	...
	// First only B,C,D,E vote: 4/5 headcount, 40% stake — under ⅔ of the stake.
	e, chainID := driveSignedAccepts(t, vs, skew, rec, blk, []int{1, 2, 3, 4})
	mustNotFinalize(t, e, blk, 2*time.Second, "40%-stake before heavy vote (accept)")
	mustNotQuasar(t, e, blk, 500*time.Millisecond, "40%-stake before heavy vote (export gate)")

	// The heavy-stake holder A (index 0) votes. 40%+60% = 100% carries the block past ⅔:
	// it is accepted and its export certificate completes in the same step.
	pos := VotePosition{ChainID: chainID, Height: blk.height, Round: 0, BlockID: blk.id, ParentID: blk.parentID}
	e.ReceiveVote(vs.signedVote(0, pos))
	mustFinalize(t, e, blk, 2*time.Second, "heavy-stake vote completes the ⅔ supermajority")
	mustQuasar(t, e, blk, 2*time.Second, "heavy-stake vote completes the ⅔ export supermajority")
}

// TestRepollIsNotAnAuthority: a re-poll firing is a LIVENESS trigger (the pollLoop re-examines
// pending blocks), a RETRY and not an authority: no amount of re-polling manufactures the
// ⅔-STAKE supermajority a block is accepted on, for a 4-of-5 head-count holding 40% of stake.
func TestRepollIsNotAnAuthority(t *testing.T) {
	...
	// Fire the re-poll finalizer repeatedly — a retry, never an authority. The ⅔
	// supermajority cannot be re-polled into existence.
	for i := 0; i < 25; i++ {
		e.processPendingBlocks()
	}
	mustNotFinalize(t, e, blk, time.Second, "40%-stake coalition under repeated re-poll (accept)")
