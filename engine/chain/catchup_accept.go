...
func (rt *Runtime) AcceptCatchupBlock(ctx context.Context, blockBytes, certBytes []byte) error {
	...
	// Already-decided heights overlap the catch-up window (the responder serves a
	// fixed ancestor window that includes blocks we already have). Skip them WITHOUT
	// verifying or tracking — they are not new work and not an error.
	if fh, set := rt.Transitive.consensus.GetFinalizedHeight(); set && blk.Height() <= fh {
		return nil
	}
