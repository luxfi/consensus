...
type Parameters struct {
	...
	SecurityLevel corona.SecurityLevel
...
func New(params Parameters) (*Engine, error) {
	rt, err := corona.New(params.SecurityLevel)
	if err != nil {
		return nil, fmt.Errorf("failed to initialize corona: %w", err)
	}
...
func (e *Engine) verifyPQ(decision Decision) error {
	...
	return e.corona.Verify(decision.Bytes(), sig)
}
...
type Decision interface {
	ID() ids.ID
	Bytes() []byte
	Signature() (corona.Signature, error)
	Verify() error
}
...
func (d *ChainDecision) Signature() (corona.Signature, error)  { return d.signature, nil }
...
func (d *DAGDecision) Signature() (corona.Signature, error)    { return d.signature, nil }
...
func (d *UnifiedDecision) Signature() (corona.Signature, error) { return d.signature, nil }
