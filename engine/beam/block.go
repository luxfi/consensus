...
var (
	...
	ErrCorona = errors.New("Corona certificate verification failed")
)

// CertBundle contains both BLS and Corona certificates
type CertBundle struct {
	BLSAgg [96]byte      // BLS aggregate signature
	RTCert []byte        // Corona certificate (nil until aggregated)
}
...
// VerifyBlock verifies both BLS and Corona certificates
func VerifyBlock(b *Block, q *quasar) error {
	...
	if !corona.VerifyCert(q.pkGroup, msg[:], b.Certs.RTCert) {
		return ErrCorona
	}
