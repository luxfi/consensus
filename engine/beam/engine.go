...
var (
	ErrMissingRTCert = errors.New("missing Corona certificate")
...
func (e *Engine) Propose(ctx context.Context, height uint64, parentID ids.ID, txs [][]byte) error {
	...
	case <-timer.C:
		...
		e.slashCh <- SlashEvent{
			Height:     height,
			ProposerID: e.nodeID,
			Reason:     "Quasar timeout - failed to collect Corona certificate",
		}
...
// OnRTShare processes incoming Corona shares
...
func (e *Engine) VerifyAndAccept(block *Block) error {
	...
	if err := VerifyBlock(block, e.quasar); err != nil {
		// If BLS passes but Corona fails, this could be a quantum attack
		if err == ErrCorona {
			// Slash the proposer for invalid Corona cert
			e.slashCh <- SlashEvent{
				Height:     block.Height,
				ProposerID: block.ProposerID,
				Reason:     "Invalid Corona certificate - possible quantum attack",
			}
		}
		return err
	}
...
// broadcastRTShare broadcasts a Corona share
func (e *Engine) broadcastRTShare(height uint64, share corona.Share) {
