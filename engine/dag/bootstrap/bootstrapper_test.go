...
// TestMaliciousVertexRejectedRetriesAnotherPeer proves a wrong/unparseable
// vertex from a faulty peer is rejected (not buffered, sync not advanced) and
// the request is retried on a different peer, after which the node converges.
func TestMaliciousVertexRejectedRetriesAnotherPeer(t *testing.T) {
	...
	m, vm, net := newManager(), &fakeVM{}, &fakeNet{}
	b, finished := newBootstrapper(t, m, vm, net, tip)
	require.NoError(t, b.Connected(ctx, bad, "v1"))
	...
	runSync(t, b, net, func(r sentReq) ([][]byte, bool) {
		...
		// The bad peer answers the first request with a wrong vertex, then with
		// outright garbage — both must be rejected.
		if !servedPoison {
			servedPoison = true
			return [][]byte{newVtx(999).Bytes()}, true // wrong id (!= requested)
		}
		return [][]byte{{0x01, 0x02, 0x03}}, true // unparseable garbage
	})

	require.True(t, servedPoison, "bad peer must have been exercised")
