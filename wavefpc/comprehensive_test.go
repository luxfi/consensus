...
// TestCoronaIntegration tests full Corona integration
func TestCoronaIntegration(t *testing.T) {
	n := 4
	f := 1
	...
	// Create with Corona enabled
	fpc := New(cfg, cls, dag, nil, validators[0], validators).(*waveFPC)

	// Should have Corona engine
	require.NotNil(t, fpc.corona)
	...
	metrics := fpc.corona.GetMetrics()
	require.Greater(t, metrics["rounds_started"], uint64(0))
}

// TestDefaultCoronaConfig tests Corona parameter defaults
func TestDefaultCoronaConfig(t *testing.T) {
	cfg := DefaultCoronaConfig(100, 33)
...
// TestCoronaHelpers tests Corona helper functions
func TestCoronaHelpers(t *testing.T) {
	n := 100
	f := 33
	...
	cfg := DefaultCoronaConfig(n, f)
	logger := &testLogger{}

	engine := NewCoronaEngine(cfg, logger, validators, validators[0])
