...
// CoronaConfig holds PQ engine configuration
...
// DefaultCoronaConfig returns default PQ configuration
func DefaultCoronaConfig(n, f int) CoronaConfig {
	return CoronaConfig{
		N:              n,
		F:              f,
...
// CoronaRound tracks a single PQ signature round
type CoronaRound struct {
	tx        TxRef
	startTime time.Time
	phase     int
...
func (r *CoronaEngine) HasPQ(tx TxRef) bool {
	_, ok := r.proofs.Get(tx)
	return ok
}

// GetPQ returns the PQ bundle if ready
func (r *CoronaEngine) GetPQ(tx TxRef) (*PQBundle, bool) {
	return r.proofs.Get(tx)
}

// runRound manages a PQ signature round
func (r *CoronaEngine) runRound(tx TxRef, round *CoronaRound, voters []ids.NodeID) {
	defer func() {
		r.roundMu.Lock()
		delete(r.rounds, tx)
		r.roundMu.Unlock()
...
Complete:
	...
	r.log.Debug("Corona PQ proof complete",
		"tx", tx,
...
func (r *CoronaEngine) generateShare(tx TxRef, phase int) *LatticeShare {
...
func (r *CoronaEngine) addShare(round *CoronaRound, share *LatticeShare) {
	r.roundMu.Lock()
	defer r.roundMu.Unlock()
...
func (r *CoronaEngine) getPhase2Shares(round *CoronaRound) map[ids.NodeID]*LatticeShare {
	r.roundMu.RLock()
...
func (r *CoronaEngine) aggregateShares(shares map[ids.NodeID]*LatticeShare) *LatticeSignature {
...
func (r *CoronaEngine) combinePhases(phase1, phase2 *LatticeSignature) *LatticeSignature {
	// Combine vectors
...
func (r *CoronaEngine) encodeSignature(sig *LatticeSignature) []byte {
...
func (r *CoronaEngine) encodeBitmap(signers []ids.NodeID) []byte {
...
// VerifyPQSignature verifies a Corona PQ signature
...
func (r *CoronaEngine) GetMetrics() map[string]uint64 {
