...
func TestCoronaBasic(t *testing.T) {
	// Setup
	n := 4
	f := 1
...
func TestCoronaWithWaveFPC(t *testing.T) {
	// Setup with Corona enabled
	n := 4
	f := 1
	...
	cfg := Config{
		...
		EnableFastPath:    true,
		EnableCorona:    true,  // Enable Corona
		EnableBLS:         true,
	...
	// Corona proof may take time to generate
...
func TestCoronaVerification(t *testing.T) {
...
// Benchmark Corona operations
func BenchmarkCoronaShareGeneration(b *testing.B) {
	n := 100
	f := 33
...
func BenchmarkCoronaAggregation(b *testing.B) {
	n := 100
	f := 33
	threshold := 2*f + 1
...
func BenchmarkCoronaVerification(b *testing.B) {
	n := 100
	f := 33
	threshold := 2*f + 1
