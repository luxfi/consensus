...
// Epoch-based Corona key management for quantum-safe consensus.
...
func (em *EpochManager) generateEpochKeysWithThreshold(epoch uint64, validators []string, threshold int) (*EpochKeys, error) {
	...
	for i, validatorID := range validators {
		...
		keys.Signers[validatorID] = coronaThreshold.NewSigner(shares[i])
	}
...
// SignBundle performs the 2-round Corona signing for a bundle.
