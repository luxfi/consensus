...
func TestRoundTripFailClosed(t *testing.T) {
	...
	// Published pulsar folds full attestation (AccountabilityQC + quorum sigs)
	// into ConsensusCert.Verify; the fail-closed path deliberately leaves the QC
	// unset (consensus owns QC aggregation, not this profile). Assert the
	// structural invariants Finalize DOES guarantee: bitmap weight == threshold
	// and every set bit indexes a validator.
	weight := 0
	for i := 0; i < len(cert.SignerBitmap)*8; i++ {
		if cert.SignerBitmap[i/8]&(1<<(uint(i)%8)) != 0 {
			if i >= s.ValidatorSetSize {
				t.Fatalf("signer bit %d out of validator set (size %d)", i, s.ValidatorSetSize)
			}
			weight++
		}
	}
	if weight != 2 {
		t.Fatalf("signer-bitmap weight = %d, want threshold 2", weight)
	}
