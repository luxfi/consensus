...
// Signer implements parallel BLS+Corona threshold signing for PQ-safe consensus.
...
func (s *signer) DualSignRound1(ctx context.Context, validatorID string, message []byte, sessionID int, prfKey []byte) (*QuasarSig, *coronaThreshold.Round1Data, error) {
	...
	go func() {
		defer wg.Done()
		round1Data, _ = s.CoronaRound1(validatorID, sessionID, prfKey)
	}()
