...
// Hybrid implements parallel BLS+Corona threshold signing for PQ-safe consensus.
// BLS provides fast classical threshold signatures.
// Corona provides post-quantum threshold signatures based on Ring-LWE.
...
func (h *Hybrid) CoronaRound1(validatorID string, sessionID int, prfKey []byte) (*coronaThreshold.Round1Data, error) {
	h.mu.RLock()
	signer, exists := h.coronaSigners[validatorID]
	h.mu.RUnlock()
	...
	signerIDs := make([]int, 0, len(h.coronaSigners))
	for _, s := range h.coronaShares {
...
func (h *Hybrid) CoronaRound2(validatorID string, sessionID int, message string, prfKey []byte, round1Data map[int]*coronaThreshold.Round1Data) (*coronaThreshold.Round2Data, error) {
	h.mu.RLock()
	signer, exists := h.coronaSigners[validatorID]
	h.mu.RUnlock()
...
func (h *Hybrid) CoronaFinalize(validatorID string, round2Data map[int]*coronaThreshold.Round2Data) (*coronaThreshold.Signature, error) {
	h.mu.RLock()
	signer, exists := h.coronaSigners[validatorID]
	h.mu.RUnlock()
...
func (h *Hybrid) VerifyCoronaSignature(message string, sig *coronaThreshold.Signature) bool {
	if h.coronaGroupKey == nil || sig == nil {
		return false
	}
	return coronaThreshold.Verify(h.coronaGroupKey, message, sig)
}
...
func (h *Hybrid) DualSignRound1(ctx context.Context, validatorID string, message []byte, sessionID int, prfKey []byte) (*HybridSignature, *coronaThreshold.Round1Data, error) {
	h.mu.RLock()
	blsSigner, hasBLS := h.blsSigners[validatorID]
	_, hasCorona := h.coronaSigners[validatorID]
	h.mu.RUnlock()
	...
	go func() {
		defer wg.Done()
		round1Data, _ = h.CoronaRound1(validatorID, sessionID, prfKey)
	}()
...
func (h *Hybrid) DualSignRound2(validatorID string, sessionID int, message string, prfKey []byte, round1Data map[int]*coronaThreshold.Round1Data) (*coronaThreshold.Round2Data, error) {
	return h.CoronaRound2(validatorID, sessionID, message, prfKey, round1Data)
}
...
// HybridSignature contains BLS and optionally Corona signature data.
...
func (h *Hybrid) IsDualThresholdMode() bool {
	return h.blsScheme != nil && h.coronaGroupKey != nil
}
...
// GenerateDualThresholdKeys generates both BLS and Corona keys (backward compat).
