...
func TestGPUOrchestrator_CoronaEnabled(t *testing.T) {
	gpu, err := NewGPUOrchestrator(DefaultGPUConfig())
	if err != nil {
		t.Fatalf("Failed to create GPU orchestrator: %v", err)
	}
	...
	enabled := gpu.CoronaGPUEnabled()
	t.Logf("Corona GPU Enabled: %v", enabled)
}

func TestGPUOrchestrator_CoronaNTT(t *testing.T) {
	gpu, err := NewGPUOrchestrator(DefaultGPUConfig())
	if err != nil {
		t.Fatalf("Failed to create GPU orchestrator: %v", err)
	}
	...
	// Forward NTT
	nttPolys, err := gpu.CoronaNTTForward(polys)
	if err != nil {
		t.Fatalf("CoronaNTTForward failed: %v", err)
	}
	if len(nttPolys) != 2 {
	...
	// Inverse NTT
	invPolys, err := gpu.CoronaNTTInverse(nttPolys)
	if err != nil {
		t.Fatalf("CoronaNTTInverse failed: %v", err)
	}
	if len(invPolys) != 2 {
		t.Errorf("expected 2 polynomials, got %d", len(invPolys))
	}

	t.Log("Corona NTT forward/inverse operations successful")
}

func TestGPUOrchestrator_CoronaPolyMul(t *testing.T) {
	gpu, err := NewGPUOrchestrator(DefaultGPUConfig())
	if err != nil {
		t.Fatalf("Failed to create GPU orchestrator: %v", err)
	}
	...
	products, err := gpu.CoronaPolyMul(aPolys, bPolys)
	if err != nil {
		t.Fatalf("CoronaPolyMul failed: %v", err)
	}
	if len(products) != 2 {
		t.Errorf("expected 2 products, got %d", len(products))
	}

	t.Log("Corona polynomial multiplication successful")
}

func TestGPUOrchestrator_CoronaPolyOperations(t *testing.T) {
	gpu, err := NewGPUOrchestrator(DefaultGPUConfig())
	if err != nil {
		t.Fatalf("Failed to create GPU orchestrator: %v", err)
	}
	...
	// Test PolyAdd
	sum, err := gpu.CoronaPolyAdd(a, b)
	if err != nil {
		t.Fatalf("CoronaPolyAdd failed: %v", err)
	}
	if len(sum) != n {
	...
	// Test PolySub
	diff, err := gpu.CoronaPolySub(a, b)
	if err != nil {
		t.Fatalf("CoronaPolySub failed: %v", err)
	}
	if len(diff) != n {
		t.Errorf("expected diff length %d, got %d", n, len(diff))
	}

	t.Log("Corona polynomial add/sub operations successful")
}

func TestGPUOrchestrator_CoronaSampling(t *testing.T) {
	gpu, err := NewGPUOrchestrator(DefaultGPUConfig())
	if err != nil {
		t.Fatalf("Failed to create GPU orchestrator: %v", err)
	}
	...
	seed := []byte("test seed for corona sampling!")

	// Test SampleUniform
	uniform, err := gpu.CoronaSampleUniform(seed)
	if err != nil {
		t.Fatalf("CoronaSampleUniform failed: %v", err)
	}
	if len(uniform) != 256 {
	...
	// Test SampleGaussian
	gaussian, err := gpu.CoronaSampleGaussian(3.2, seed)
	if err != nil {
		t.Fatalf("CoronaSampleGaussian failed: %v", err)
	}
	if len(gaussian) != 256 {
	...
	// Test SampleTernary
	ternary, err := gpu.CoronaSampleTernary(0.5, seed)
	if err != nil {
		t.Fatalf("CoronaSampleTernary failed: %v", err)
	}
	if len(ternary) != 256 {
		t.Errorf("expected ternary length 256, got %d", len(ternary))
	}

	t.Log("Corona sampling operations successful")
}

func TestGPUOrchestrator_CoronaMatrixVectorMul(t *testing.T) {
	gpu, err := NewGPUOrchestrator(DefaultGPUConfig())
	if err != nil {
		t.Fatalf("Failed to create GPU orchestrator: %v", err)
	}
	...
	result, err := gpu.CoronaMatrixVectorMul(matrix, vector)
	if err != nil {
		t.Fatalf("CoronaMatrixVectorMul failed: %v", err)
	}
	if len(result) != 2 {
		t.Errorf("expected result length 2, got %d", len(result))
	}

	t.Log("Corona matrix-vector multiplication successful")
}

func BenchmarkGPU_CoronaNTT(b *testing.B) {
	gpu, err := NewGPUOrchestrator(DefaultGPUConfig())
	if err != nil {
		b.Fatalf("Failed to create GPU orchestrator: %v", err)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = gpu.CoronaNTTForward(polys)
	}
}

func BenchmarkGPU_CoronaPolyMul(b *testing.B) {
	gpu, err := NewGPUOrchestrator(DefaultGPUConfig())
	if err != nil {
		b.Fatalf("Failed to create GPU orchestrator: %v", err)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = gpu.CoronaPolyMul(aPolys, bPolys)
	}
}
