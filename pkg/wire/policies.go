...
func (p *QuantumPolicy) OnVote(ctx context.Context, vote *Vote) error {
	...
	case SigQuasar:
		// Quasar vote contains both: [scheme][bls_len][bls][corona]
...
func (p *QuantumPolicy) MaybeFinalize(ctx context.Context, candidateID CandidateID) (*Certificate, error) {
	...
	// Build Quasar proof: [bls_aggregate][corona_signatures...]
	// In production: aggregate BLS signatures, concat Corona signatures
	proof := make([]byte, 0)
