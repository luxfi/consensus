...
// PQFinalizer implements parallel Corona post-quantum finality
...
func NewPQFinalizer(config CoronaConfig, corona ThresholdKey, validators []ids.NodeID) *PQFinalizer {
	return &PQFinalizer{
		config:      config,
		corona:    corona,
...
func (pq *PQFinalizer) OnMetastableFinality(height uint64, blockHash ids.ID) {
	...
	// Launch parallel Corona signing
...
func (pq *PQFinalizer) collectQuantumSignatures(height uint64, blockHash ids.ID) {
	...
	// Sign with our share
	share, err := pq.corona.Sign(message)
	if err != nil {
...
func (pq *PQFinalizer) waitForThreshold(height uint64) {
	threshold, _ := pq.corona.GetThreshold()
...
func (pq *PQFinalizer) createQuantumCertificate(height uint64) {
	...
	cert, err := pq.corona.Aggregate(shares)
	if err != nil {
