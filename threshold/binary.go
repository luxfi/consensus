...
// binaryThreshold implements binary threshold consensus
...
func (sf *binaryThreshold) RecordPoll(count, choice int) {
	if sf.finalized {
	...
	sf.BinarySampler.RecordSuccessfulPoll(choice)
...
func (sf *binaryThreshold) RecordUnsuccessfulPoll() {
	clear(sf.confidence)
}

func (sf *binaryThreshold) Finalized() bool {
	return sf.finalized
}

func (sf *binaryThreshold) String() string {
	return fmt.Sprintf("SF(Confidence = %v, Finalized = %v, %v)",
		sf.confidence,
		sf.finalized,
		&sf.BinarySampler)
}
