...
func (s *vdrSet) prefixedString(prefix string) string {
	...
	format := fmt.Sprintf("\n%s    Validator[%%s]: %%s, %%d", prefix)
	for i, vdr := range s.vdrSlice {
		sb.WriteString(fmt.Sprintf(
			format,
			formatting.IntFormat(i),
			vdr.NodeID.String(),
			vdr.Weight,
		))
	}
