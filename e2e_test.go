...
func TestE2EConsensusServer(t *testing.T) {
	...
	t.Run("Status", func(t *testing.T) {
		...
		if resp.StatusCode != http.StatusOK {
			t.Errorf("Status endpoint failed: status %d", resp.StatusCode)
		}
		...
		if healthyVal, ok := status["healthy"]; ok {
			if healthy, ok := healthyVal.(bool); ok {
				...
				t.Errorf("Healthy field is not a boolean: %T", healthyVal)
			}
		} else {
			t.Error("Status response missing 'healthy' field")
		}
	...
	t.Run("ConsensusRound", func(t *testing.T) {
		...
		if resp.StatusCode != http.StatusOK {
			t.Errorf("Consensus endpoint failed: status %d", resp.StatusCode)
		}
