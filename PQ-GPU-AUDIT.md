...
| `accel.HashOps` | `lux/accel/ops*.go` | Keccak / SHA3 batch (Pulsar / Comet hash chain) |
...
| Threshold sig | Comet | threshold SLH-DSA (FIPS 205) |
| Cert profile | Pulsar / Aurora / Magnetar | floor / Pulsar∥Corona / Pulsar∥Corona∥Comet |
...
| **Comet** (threshold SLH-DSA) | per-validator SLH-DSA at `crypto/slhdsa/`, batch dispatch via `crypto/slhdsa/gpu.go` | `VerifyBatch` / `VerifyBatchGPU` route through `crypto/backend.Resolve` → `accel.LatticeOps.SLHDSAVerifyBatch` → luxcpp `lux_slhdsa_verify_batch` C ABI → (Metal/CUDA plugin when registered) `luxcpp/crypto/slhdsa/gpu/{metal,cuda}/`. Stub returns `LUX_NO_BACKEND` until a backend plugin loads; transparent fall-back to per-element CPU verify. | `luxcpp/crypto/slhdsa/gpu/{metal,cuda}/slhdsa.{metal,cu}` (kernel substrate) **plus** `lux_slhdsa_{sign,verify}_batch` in `luxcpp/lux-accel/include/lux/accel/c_api.h` + `src/c_api.cpp` (this pass) | **Shipped** in this pass: (a) `lux_slhdsa_{sign,verify}_batch` C entry points landed in `lux-accel` v0.1.1 (dylib re-cut, ad-hoc linker-signed at `/opt/homebrew/lib/libluxaccel.0.1.0.dylib`); (b) `accel.LatticeOps.SLHDSA{Sign,Verify}Batch` added in `lux/accel` (cgo binding + non-cgo stub + weak-symbol fallback in `internal/capi/stub.go`); (c) `crypto/slhdsa/gpu.go` mirrors `crypto/mldsa/gpu.go` with `VerifyBatchGPU` + `VerifyBatch` public entry points; (d) `gpu_test.go` pins CPU/GPU equivalence (trivial when no plugin loaded; real on a GPU-equipped host with a plugin) + tampered-sig rejection + empty-batch boundary + batch-of-21 throughput bench. Real GPU kernel registration is a backend-plugin concern (separate ship vehicle); the wire is verified end-to-end. |
...
4. **`crypto/slhdsa/gpu.go` + `accel.LatticeOps.SLHDSA*` + luxcpp
   `lux_slhdsa_*` C entry points**: this is the Comet GPU wire. Three-
...
The only true gap is Comet, and that's blocked on the lux-accel C
API extension.
...
After the new SLH-DSA batch dispatch lands, `bls-comet` mode would
...
applies to ML-DSA batch verify; Comet/SLH-DSA threshold will be
...
| Naming-lock drift | Audit doc anchors the lock — Pulsar (threshold ML-DSA), Corona (threshold Ring-LWE), Comet (threshold SLH-DSA), P3Q (Z-Chain STARK), Quasar (engine). Cert profiles Pulsar / Aurora / Magnetar. No Wing names, no Ringtail brand on the consensus surface. |
...
| Comet (SLH-DSA batch verify) | CPU only, no batch path | **Shipped** — GPU-batch via `accel.LatticeOps.SLHDSA{Sign,Verify}Batch` + `crypto/slhdsa/gpu.go` `VerifyBatch{,GPU}`. Wire end-to-end; substrate kernel registration deferred to backend-plugin layer. |
| Comet (full threshold-SLH-DSA primitive) | Not implemented as a Go pkg | Out of scope this pass — primitive needs ITC threshold-hash construction work; opens later. Comet cert-profile (Magnetar) is unblocked once primitive lands. |
...
The full Comet (threshold SLH-DSA) primitive is the only remaining
...
its own LP and its own dedicated push when the spec lands. **Comet
batch verify (this pass)** ≠ **Comet threshold primitive (future
...
- **`PULSAR_BACKEND`, `CORONA_BACKEND`, `COMET_BACKEND`, `QUASAR_BACKEND` env vars**: rejected for the same reason. `CRYPTO_BACKEND` is canonical.
