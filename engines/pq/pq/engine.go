...
import (
	...
	"github.com/luxfi/consensus/corona"
...
// It combines Nova DAG with Corona PQ for both classical and quantum security.
type Engine struct {
	mu sync.RWMutex
	...
	// Corona components (quantum finality)
	certManager *corona.CertificateManager
	validators  corona.ValidatorSet

	// Keys
	blsKey interface{} // BLS key for classical finality
	rtKey  interface{} // Corona key for quantum finality
	...
	certificates map[uint64]*corona.CertBundle // height -> certificates
...
func New(params config.Parameters, nodeID ids.NodeID) *Engine {
	return &Engine{
		...
		certificates: make(map[uint64]*corona.CertBundle),
	}
}

// Initialize initializes the engine with keys and validators.
func (e *Engine) Initialize(ctx context.Context, blsKey, rtKey interface{}, validators corona.ValidatorSet) error {
	e.mu.Lock()
	defer e.mu.Unlock()
	...
	e.certManager = corona.NewCertificateManager(e.nodeID, blsKey, rtKey, validators)
...
// createCertificates creates both BLS and Corona certificates for an item.
func (e *Engine) createCertificates(item *Item) {
	...
	item.CertBundle = &corona.CertBundle{
		BLSAgg: blsSig,
...
// ProcessRTShare processes a Corona share from another validator.
func (e *Engine) ProcessRTShare(round uint64, share corona.Share) error {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *Engine) GetCertBundle(height uint64) (*corona.CertBundle, bool) {
	e.mu.RLock()
	defer e.mu.RUnlock()
