...
type Quasar struct {
	...
	// Corona PQ overlay for quantum security
	corona *Corona
...
type QBlock struct {
	...
	PQCertificate []byte            // Corona PQ certificate
...
func NewQuasar(params config.Parameters, nodeID ids.NodeID) *Quasar {
	return &Quasar{
		...
		corona:            NewCorona(params, nodeID),
...
func (q *Quasar) RecordPoll(ctx context.Context, nodeID ids.NodeID, votes []ids.ID) error {
	...
	// Check if we should start a new Corona round
	q.checkCoronaTrigger(ctx)

	return nil
}

// RecordCoronaMessage handles Corona protocol messages.
func (q *Quasar) RecordCoronaMessage(ctx context.Context, msg interface{}) error {
	q.mu.Lock()
	defer q.mu.Unlock()

	switch m := msg.(type) {
	case *CoronaProposal:
		return q.corona.RecordProposal(ctx, m)
	case *CoronaCommit:
		return q.corona.RecordCommit(ctx, m)
	default:
		return fmt.Errorf("unknown Corona message type: %T", msg)
	}
}
...
// checkCoronaTrigger checks if we should start a new Corona round
func (q *Quasar) checkCoronaTrigger(ctx context.Context) {
	q.mu.Lock()
	defer q.mu.Unlock()

	// Only start if idle
	if q.corona.GetPhase() != PhaseIdle {
		return
	}
	...
	// Start Corona round
	if err := q.corona.StartRound(ctx, vertex); err != nil {
		// Log error
		return
	}
}
...
func (q *Quasar) checkEventHorizonCrossing(ctx context.Context) {
	...
	// Check if Corona finalized
	_, commit, ok := q.corona.GetFinalized()
	if !ok {
		return
	}
	...
	// Reset Corona for next round
	q.corona = NewCorona(q.params, q.nodeID)
}
...
func (q *Quasar) GetStats() Stats {
	...
	stats := Stats{
		...
		CoronaPhase:      q.corona.GetPhase(),
		CoronaRound:      q.corona.GetRound(),
