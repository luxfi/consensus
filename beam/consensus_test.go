...
func InitializeTest(t *testing.T, factory Factory) {
	...
	require.NoError(sm.Initialize(
		...
		params,
		beamtest.GenesisID,
		beamtest.GenesisHeight,
		beamtest.GenesisTimestamp,
	))

	require.Equal(beamtest.GenesisID, sm.Preference())
...
func AddToNonTailTest(t *testing.T, factory Factory) {
	...
	firstBlock := beamtest.BuildChild(beamtest.Genesis)
	secondBlock := beamtest.BuildChild(beamtest.Genesis)
...
func StatusOrProcessingPreviouslyAcceptedTest(t *testing.T, factory Factory) {
	...
	pref, ok := sm.PreferenceAtHeight(beamtest.Genesis.Height())
	require.True(ok)
	require.Equal(beamtest.Genesis.ID(), pref)
}
...
func RecordPollWhenFinalizedTest(t *testing.T, factory Factory) {
	...
	votes := bag.Of(beamtest.GenesisID)
	require.NoError(sm.RecordPoll(context.Background(), votes))
	require.Zero(sm.NumProcessing())
	require.Equal(beamtest.GenesisID, sm.Preference())
}
...
func RecordPollRejectTransitivelyTest(t *testing.T, factory Factory) {
	...
	block0 := beamtest.BuildChild(beamtest.Genesis)
	block1 := beamtest.BuildChild(beamtest.Genesis)
	block2 := beamtest.BuildChild(block1)
...
func RecordPollTransitivelyResetConfidenceTest(t *testing.T, factory Factory) {
	...
	block1 := beamtest.BuildChild(beamtest.Genesis)
	block2 := beamtest.BuildChild(block1)
	block3 := beamtest.BuildChild(block1)
...
func RecordPollTransitiveVotingTest(t *testing.T, factory Factory) {
	...
	block0 := beamtest.BuildChild(beamtest.Genesis)
	block1 := beamtest.BuildChild(block0)
	block2 := beamtest.BuildChild(block1)
	block3 := beamtest.BuildChild(block0)
	block4 := beamtest.BuildChild(block3)
...
func RecordPollDivergedVotingWithNoConflictingBitTest(t *testing.T, factory Factory) {
	...
	block3 := beamtest.BuildChild(block2)
	...
	// Although we are adding in [block2] here - the underlying focus
	// instance has already decided it is rejected. Beam doesn't actually
	// know that though, because that is an implementation detail of the
	// Focus trie that is used.
	require.NoError(sm.Add(block2))
...
func RecordPollChangePreferredChainTest(t *testing.T, factory Factory) {
	...
	a1Block := beamtest.BuildChild(beamtest.Genesis)
	b1Block := beamtest.BuildChild(beamtest.Genesis)
	a2Block := beamtest.BuildChild(a1Block)
	b2Block := beamtest.BuildChild(b1Block)
...
func LastAcceptedTest(t *testing.T, factory Factory) {
	...
	block1 := beamtest.BuildChild(block0)
	block2 := beamtest.BuildChild(block1)
	block1Conflict := beamtest.BuildChild(block0)

	lastAcceptedID, lastAcceptedHeight := sm.LastAccepted()
	require.Equal(beamtest.GenesisID, lastAcceptedID)
	require.Equal(beamtest.GenesisHeight, lastAcceptedHeight)
...
func RecordPollWithDefaultParameters(t *testing.T, factory Factory) {
	...
	blk1 := beamtest.BuildChild(beamtest.Genesis)
	blk2 := beamtest.BuildChild(beamtest.Genesis)
...
func RecordPollRegressionCalculateInDegreeIndegreeCalculation(t *testing.T, factory Factory) {
	...
	blk1 := beamtest.BuildChild(beamtest.Genesis)
	blk2 := beamtest.BuildChild(blk1)
	blk3 := beamtest.BuildChild(blk2)
