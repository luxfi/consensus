...
Lux Quasar consensus achieves quantum-resistant finality through dual certificates - every block requires both BLS (classical) and Corona (post-quantum) signatures. This document details the implementation across the consensus and node packages.
...
1. **PQ Engine** (`engine/pq/`)
   ...
   - Integrates Nova (classical) + Corona (quantum)

2. **Corona Layer** (`corona/`)
   - `certificate.go` - CertBundle with BLS + RT certificates
   - `keys.go` - Corona key management
...
The Quasar service precomputes Corona shares to hide lattice computation latency:
...
The dual-certificate implementation ensures Lux remains secure against both classical and quantum attacks. By requiring both BLS and Corona signatures for every block, we achieve:
