...
|------|--------------|-------|-------|
| 0x00 | Q_LAST_HEIGHT | uint256 – latest block height covered by a valid Corona certificate | monotonic |
...
| 0x03 | Q_THRESHOLD_T | uint256 – Corona threshold t (e.g. 15) | constant after genesis |
| 0x04 | Q_PUBLIC_KEY_HASH | bytes32 – hash of Corona group public key | constant |
...
interface IQuasar {
    ...
    /// @param rtCert      Corona threshold certificate blob
...
│     ├─ call pre-linked Corona C-lib (or BN128 FFI) :
│          ok = corona_verify(pubkey, rtCert, M, t)
│     └─ require(ok == true)
│
├── 5.  Update state
│     ├─ SSTORE slot0 = height
...
Corona verification is written in native Go (inside Subnet-EVM) and linked into the precompile just like ecrecover is in geth.
...
| Corona lattice verify | 170,000–230,000 (depends on n,t) |
...
- **Parallelism** – Corona verification is native code executed inside the precompile, not by Solidity; runs in < 3 ms on a single core, well below block times.
...
- **BLS + Corona dual-cert** is enforced at consensus.
