...
//	PQModeBLSPlusCorona -- BLS + Corona threshold (O(1) PQ in cert)
//	PQModeBLSPlusGroth16  -- BLS + 192 B Groth16 ML-DSA rollup (Z-Chain)
//	PQModeTripleQuantum   -- BLS + Corona + ML-DSA (defense in depth)
type PQMode uint8
...
const (
	...
	// PQModeBLSPlusCorona pairs BLS with the Corona (Ring-LWE) threshold
	...
	// PQModeTripleQuantum runs BLS + Corona + ML-DSA in parallel.
...
//	PQModeBLSPlusCorona -> 5 (PolicyPQ, P+Q witness set)
...
//	"bls-rt", "bls-q" -> PQModeBLSPlusCorona
...
func ParsePQMode(s string) (PQMode, error) {
	...
	case "bls-rt", "bls-q", "rt", "corona", "bls+corona":
		return PQModeBLSPlusCorona, nil
